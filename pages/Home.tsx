...
const Home: React.FC = () => {
  ...
  const fetchDishes = async () => {
    // 1. Check local storage first (simulating backend updates from Admin panel)
    const localDishes = localStorage.getItem('zhulebino_dishes');
    if (localDishes) {
        try {
            setDishes(JSON.parse(localDishes));
            return;
        } catch (e) {
            console.error("Error parsing local dishes", e);
        }
    }
    ...
    if (error || !data || data.length === 0) {
      console.log('Using mock dishes data');
      setDishes(MOCK_DISHES);
      // Initialize local storage so admin has something to edit
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="text-center py-6">
        <h1 className="text-4xl font-bold text-amber-900 mb-2 font-serif">Меню Чайханы</h1>
        <p className="text-amber-700/80">Лучшие вкусы востока у вас дома</p>
      </div>
        ...
        {selectedDish && (
          <div className="space-y-6">
              ...
              <div className="flex items-center justify-between bg-amber-50 p-4 rounded-xl">
                ...
                {selectedDish.available ? (
                    ...
                    <button 
                        ...
                        className="w-8 h-8 flex items-center justify-center text-amber-900 hover:bg-amber-50 rounded font-bold"
                    >
                        -
                    </button>
            ...
            <button 
              ...
              className="w-full bg-amber-900 text-white py-3.5 rounded-xl font-bold text-lg hover:bg-amber-800 active:scale-[0.98] transition shadow-lg shadow-amber-900/20 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {selectedDish.available 
                ? `Добавить в корзину (${selectedDish.price * quantity} ₽)`
                : 'Нет в наличии'}
            </button>
          </div>
        )}
